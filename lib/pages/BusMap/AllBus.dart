...
class AllBusTrackState extends State<AllBusTrack> {
  ...
  getIcons() {
    BitmapDescriptor.fromAssetImage(ImageConfiguration(devicePixelRatio: 3.2), "images/marker-re.png").then((icon) {
      setState(() {
        customeIcon = icon;
      });
    },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
    body: StreamBuilder(stream: live_ref.onValue,builder: (context, AsyncSnapshot<DatabaseEvent> snapshot) {
          ...
          if (snapshot.hasData && !snapshot.hasError && snapshot.data != null) {
            ...
            map.forEach((key, value) {
              ...
              infoWindow: InfoWindow( //popup info 
                title: key.toString(),
                snippet: "Speed - ${value["speed"]}km | Satellite - ${value["satellite"]}",
