...
class _AssignSuperiorState extends State<AssignSuperior> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        Container(padding: EdgeInsets.all(20.0),child: Form(
          key: _formkey,
          child: Column(
            
            children: <Widget>[
              const SizedBox(height: 30.0),
              Text("Assign Department HOD", 
              style: TextStyle(
                  fontSize: 24,
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  dynamic result = await adminDB.setHodFacultyRole(facultyNameHod,facultyNameHodDept);
                  if (result != null) {
                      ScaffoldMessenger.of(context).showSnackBar(getSnackBar(result.toString()));
                  }
              ...
              Divider(height: 20,
                    ...
                    endIndent: 1,),
              const SizedBox(height: 30.0),
              Text("Assign Transport Incharge", 
              style: TextStyle(
                  fontSize: 24,
                  fontFamily: 'Times New Roman',
              ),),
              ...
              ElevatedButton(
                style: ElevatedButton.styleFrom(
                    ...
                    padding: const EdgeInsets.fromLTRB(25, 8, 25, 8)),
                onPressed: () async {
                  dynamic result = await adminDB.setTransportInchargeFacultyRole(facultyNameBusIncharge);
                  if (result != null) {
                      ScaffoldMessenger.of(context).showSnackBar(getSnackBar(result.toString()));
                  }
                  // if (_formkey.currentState!.validate()) {
              ...
              ElevatedButton(
                style: ElevatedButton.styleFrom(
                    ...
                    padding: const EdgeInsets.fromLTRB(25, 8, 25, 8)),
                onPressed: () async {
                  dynamic result = await adminDB.setSuperAdminRole(facultyNameBusIncharge);
                  if (result != null) {
                      ScaffoldMessenger.of(context).showSnackBar(getSnackBar(result.toString()));
                  }
                  // if (_formkey.currentState!.validate()) {
